...
## S0: Create Autoscaling using AWS Management Console
...
## S1: Introduction to Autoscaing using Terraform
### Create ASG & LC & ALB
...
### Create TTSP (Target Tracking Scaling Policies)
...
### TF Changes to ASG - Test Instance Refresh
...
### Change to Launch Configuration - Test Instance Refresh
- What happens?
...
### Testing using Postman for Autoscaling
...
3. local-values.tf: ADDED `asg_tags`
4. VPC Module
- vpc-variables.tf
- vpc-module.tf
- vpc-outputs.tf
...
- securitygroup-loadbalancersg.tf
6. Datasources
- datasource-ami.tf
- datasource-route53-zone.tf
7. EC2 Instance Module
- ec2instance-variables.tf
- ec2instance-outputs.tf: REMOVED OUTPUTS RELATED TO OTHER PRIVATE EC2 INSTANCES
- ec2instance-bastion.tf
8. elasticip.tf
9. nullresource-provisioners.tf
10. Application Load Balancer Module
- ALB-application-loadbalancer-variables.tf
- ALB-application-loadbalancer.tf: CHANGES RELATED TO APP1 TG, REMOVE TARGETS, TARGETS WILL BE ADDED FROM ASG
- ALB-application-loadbalancer-outputs.tf
11. acm-certificatemanager.tf
12. route53-dnsregistration.tf: JUST CHANGED THE DNS NAME
13. Autoscaling with Launch Configuration Module: NEW ADDITION
- autoscaling-with-launchconfiguration-variables.tf
- autoscaling-additional-resoures.tf
- autoscaling-with-launchconfiguration.tf
- autoscaling-with-launchconfiguration-outputs.tf
- autoscaling-notifications.tf
- autoscaling-ttsp.tf
- autoscaling-scheduled-actions.tf
...
## S5: ALB-application-loadbalancer.tf
- Two changes
- **Change-1:** For `subnets` argument, either we can give specific subnets or we can also give all private subnets defined. 
- **Change-2:** Removed the Targets for App1/2/3, App1 Targets now will be added automatically from ASG. HOW?
...
## S6: route53-dnsregistration.tf
- Update the DNS name relevant to demo
```t
  # DNS Registration 
resource "aws_route53_record" "apps_dns" {
  zone_id = data.aws_route53_zone.mydomain.zone_id
  name    = "asg-launchconfig1.kishorekumar.online"
  type    = "A"
  alias {
...
## S 7: Autoscaling with Launch Configuration Terraform Module
### S7-01: autoscaling-with-launchconfiguration-variables.tf
```t
# Autoscaling Input Variables
## Placeholder file
```

### S7-02: autoscaling-additional-resoures.tf
```t
...
### S7-03: autoscaling-with-launchconfiguration.tf
```t
...
### S7-04: autoscaling-with-launchconfiguration-outputs.tf
```t
...
### S7-05: autoscaling-notifications.tf
#### S7-05-01: versions.tf
```t
...
#### S7-05-02: autoscaling-notifications.tf
```t
# Autoscaling Notifications
## SNS - Topic
...
### S7-07: autoscaling-scheduled-actions.tf
#### S7-07-01: Terraform Import Command
```t
# Import State
$ terraform import aws_autoscaling_schedule.resource-name auto-scaling-group-name/scheduled-action-name
terraform import aws_autoscaling_schedule.capacity_increase_during_business_hours	 myapp1-asg-20210329100544375800000007/capacity_increase_during_business_hours	
-> using terraform import get values for recurrence argument (cron format)
```
#### S7-07-02: ASG Scheduled Actions
...
## S9: Verify the AWS resources created
...
```t
# Access and Test
http://asg-lc.kishorekumar.online
http://asg-lc.kishorekumar.online/app1/index.html
http://asg-lc.kishorekumar.online/app1/metadata.html
```


## S10: Changes to ASG - Test Instance Refresh
- Change Desired capacity to 3 `desired_capacity = 3` and test
- Any change to ASG specific arguments listed in `triggers` of `instance_refresh` block, do a instance refresh
```t
  # ASG Instance Referesh
...
## S11: Change to Launch Configuration - Test Instance Refresh
- In next scale-in event changes will be adjusted [or] if instance refresh present and configured in this module it updates ASG with new LC ID, instance refresh should kick in.
...
3. Verify EC2 Instances, old will be terminated and new will be created
```
## S12: Test Autoscaling using Postman
...
- URL: https://asg-lc.kishorekumar.online/app1/metadata.html
