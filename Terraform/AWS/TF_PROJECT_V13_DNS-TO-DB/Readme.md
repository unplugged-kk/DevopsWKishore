...
description: Create a DNS to DB Demo test on AWS with Route53, ALB, EC2 and RDS Database with 3 Applications
---
...
###  Create RDS Database Terraform Configs
...
### Create EC2 Instance Terraform Configs
...
### Create Jumpbox server to have mysql client installed
...
### Create DNS Name AWS Route53 Record Set
- Give `dns-to-db` DNS name for Route53 record



## Terraform RDS Database Configurations
- Create RDS DB Security Group
- Create RDS DB Variables with `sensitive` argument for DB password
- Create RDS DB Module
- Create RDS DB Outputs
### securitygroup-rdsdbsg.tf
...
## Create new EC2 Instance Module for App3 UMS
...
```t
# Change Directory 
cd TF_PROJECT_V13_DNS-TO-DB/templatefile-function-demo
# Terraform Console
terraform console
...
wget https://github.com/unplugged-kk/sample-usermgmt-app/releases/download/1.0.0/usermgmt-webapp.war -P /home/ec2-user/app3-usermgmt 
...
```t
    # App3 Target Group - TG Index = 2
    {
      ...
      targets = {
        my_app3_vm1 = {
          target_id = module.ec2_private_app3["vm1"].id
          port      = 8080
        },
        my_app3_vm2 = {
          target_id = module.ec2_private_app3["vm1"].id
...
```
### Create Listener Rules for App3
```t
...
```
### Implement Rule Priority for all 3 Listener Rules
- Listener Rule Priorities
- **/app1*:** `priority = 1`
- **/app2*:** `priority = 2`
- **Root Context /*:** `priority = 3`

## Automate Jumpbox server to have mysql client installed
- Using jumpbox userdata, `mysql client` should be auto-installed.
...
```t
  name    = "dns-to-db.kishorekumar.online"
```
...
```t
# App1
https://dns-to-db.kishorekumar.online/app1/index.html

# App2
https://dns-to-db.kishorekumar.online/app2/index.html

# App3
https://dns-to-db.kishorekumar.online
Username: admin101
...
## Additional Troubleshooting for App3
